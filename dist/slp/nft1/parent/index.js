...
export default class SlpNft1Parent {
    ...
    async getNftUtxos(handle, tokenId, rawTotalSendAmount, isCreatingChildNft, groupBaton = false) {
        ...
        if (handle.indexOf('wallet:') > -1) {
            resp = await this._api.get(`utxo/wallet/${handle.split('wallet:')[1]}/${tokenId}/?&value=${rawTotalSendAmount}&baton=${groupBaton}`);
        }
        else {
            resp = await this._api.get(`utxo/slp/${handle}/${tokenId}/?&value=${rawTotalSendAmount}&baton=${groupBaton}`);
        }
        ...
        const utxos = resp.data.utxos.filter(u => {
            ...
            if (groupBaton)
                return u.token_type === this.tokenType && u.amount === 0;
    ...
    async generateMintingBatonUtxo({ sender, feeFunder, groupTokenId, recipient, changeAddress, broadcast }) {
    ...
    async createChildNft({ sender, feeFunder, groupTokenId, recipient, changeAddress, broadcast, label, ticker, docUrl = '' }) {
        return await this.createChildNftOrMintingBatonUtxo({
            sender,
            feeFunder,
            groupTokenId,
    ...
    async findChildNftMintingBaton({ groupTokenId, walletHash, address, }) {
    ...
    async createChildNftOrMintingBatonUtxo({ sender, feeFunder, groupTokenId, recipient, changeAddress, broadcast, label, ticker, docUrl, isChildNft = true }) {
        let walletHash;
        ...
        transactionBuilder.addOutput(bchjs.SLP.Address.toLegacyAddress(recipient), this.dustLimit);
        if (hasNftGroupChange) {
            transactionBuilder.addOutput(bchjs.SLP.Address.toLegacyAddress(sender.address), this.dustLimit);
    ...
    async create({ creator, feeFunder, initialMintRecipient, mintBatonRecipient, changeAddress, broadcast, name, ticker, initialQty, docUrl = '', fixedSupply = false }) {
    ...
    async mint({ minter, feeFunder, tokenId, quantity, additionalMintRecipient, mintBatonRecipient, changeAddress, broadcast, passMintingBaton = true }) {
        ...
        const nftMintData = nftOpRetGen.generateGroupMintOpReturn(nftUtxos.utxos, quantity);
