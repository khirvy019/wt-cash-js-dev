...
export default class SlpType1 {
    ...
    async getSlpUtxos(handle, tokenId, rawTotalSendAmount, baton = false, burn = false) {
        ...
        if (handle.indexOf('wallet:') > -1) {
            resp = await this._api.get(`utxo/wallet/${handle.split('wallet:')[1]}/${tokenId}/?value=${rawTotalSendAmount}&baton=${baton}`);
        }
        else {
            resp = await this._api.get(`utxo/slp/${handle}/${tokenId}/?value=${rawTotalSendAmount}&baton=${baton}`);
        }
    ...
    async send({ sender, feeFunder, tokenId, recipients, changeAddresses, broadcast, burn = false }) {
        let walletHash;
        ...
        const slpUtxos = await this.getSlpUtxos(handle, tokenId, totalTokenSendAmounts, false, burn);
        try {
        ...
        for (let i = 0; i < slpUtxos.utxos.length; i++) {
            ...
            if (walletHash) {
                ...
                const utxoPkWif = await this.retrievePrivateKey(sender.mnemonic, sender.derivationPath, addressPath);
        ...
        if (burn) {
            ...
            slpOpRetData = slpOpRetGen.generateBurnOpReturn(slpUtxos.utxos, quantity);
        }
        else {
        ...
        const vm = this;
        recipients.map(function (recipient) {
            transactionBuilder.addOutput(bchjs.SLP.Address.toLegacyAddress(recipient.address), vm.dustLimit);
            outputsCount += 1;
            totalOutputSats = totalOutputSats.plus(vm.dustLimit);
        });
        if (tokenRemainder.isGreaterThan(0)) {
            transactionBuilder.addOutput(bchjs.SLP.Address.toLegacyAddress(changeAddresses.slp), this.dustLimit);
            outputsCount += 1;
        ...
        for (let i = 0; i < bchUtxos.utxos.length; i++) {
            ...
            if (feeFunder.walletHash) {
                ...
                const utxoPkWif = await this.retrievePrivateKey(feeFunder.mnemonic, feeFunder.derivationPath, addressPath);
        ...
        if (remainderSats.isGreaterThanOrEqualTo(this.dustLimit)) {
            transactionBuilder.addOutput(bchjs.Address.toLegacyAddress(changeAddresses.bch), parseInt(remainderSats));
        }
        else {
        ...
        for (let i = 0; i < combinedUtxos.length; i++) {
            ...
            transactionBuilder.sign(i, keyPairs[i], redeemScript, transactionBuilder.hashTypes.SIGHASH_ALL, parseInt(utxo.value));
        }
    ...
    async create({ creator, feeFunder, initialMintRecipient, mintBatonRecipient, changeAddress, broadcast, name, ticker, decimals, initialQty, docUrl = '', docHash = '', fixedSupply = false, isNftParent = false }) {
        if (fixedSupply) {
            if (initialQty < 1) {
                return {
                    success: false,
        ...
        for (let i = 0; i < bchUtxos.utxos.length; i++) {
            ...
            if (walletHash) {
                ...
                const utxoPkWif = await this.retrievePrivateKey(creator.mnemonic, creator.derivationPath, addressPath);
                utxoKeyPair = bchjs.ECPair.fromWIF(utxoPkWif);
                keyPairs.push(utxoKeyPair);
                if (!changeAddress) {
        ...
        if (isNftParent) {
            ...
            slpCreateData = await nftOpRetGen.generateGroupCreateOpReturn(fixedSupply, name, ticker, docUrl, initialQty);
        }
        else {
            slpCreateData = await slpOpRetGen.generateGenesisOpReturn(fixedSupply, name, ticker, decimals, initialQty, docUrl, docHash);
        }
        transactionBuilder.addOutput(slpCreateData, 0);
        transactionBuilder.addOutput(bchjs.SLP.Address.toLegacyAddress(initialMintRecipient), this.dustLimit);
        outputsCount += 2;
        if (!fixedSupply) {
            transactionBuilder.addOutput(bchjs.SLP.Address.toLegacyAddress(mintBatonRecipient), this.dustLimit);
            outputsCount += 1;
        }
        ...
        if (feeFunder !== undefined) {
            ...
            if (creatorRemainder.isGreaterThanOrEqualTo(this.dustLimit)) {
                transactionBuilder.addOutput(bchjs.Address.toLegacyAddress(changeAddress), parseInt(creatorRemainder));
            }
            else {
            ...
            if (feeFunderRemainder.isGreaterThan(this.dustLimit)) {
                transactionBuilder.addOutput(bchjs.SLP.Address.toLegacyAddress(feeFunder.address), parseInt(feeFunderRemainder));
            }
            else {
    ...
    async mint({ minter, feeFunder, tokenId, quantity, additionalMintRecipient, mintBatonRecipient, changeAddress, broadcast, passMintingBaton = true }) {
        if (quantity < 1) {
            return {
                success: false,
        ...
        for (let i = 0; i < slpUtxos.utxos.length; i++) {
            ...
            if (walletHash) {
                ...
                const utxoPkWif = await this.retrievePrivateKey(minter.mnemonic, minter.derivationPath, addressPath);
                utxoKeyPair = bchjs.ECPair.fromWIF(utxoPkWif);
            }
            else {
        ...
        const slpMintData = slpOpRetGen.generateMintOpReturn(slpUtxos.utxos, quantity);
        transactionBuilder.addOutput(slpMintData, 0);
        transactionBuilder.addOutput(bchjs.SLP.Address.toLegacyAddress(additionalMintRecipient), this.dustLimit);
        totalOutputSats = totalOutputSats.plus(this.dustLimit);
        outputsCount += 2;
        if (passMintingBaton) {
            transactionBuilder.addOutput(bchjs.SLP.Address.toLegacyAddress(mintBatonRecipient), this.dustLimit);
        ...
        const remainderSats = totalInputSats.minus(totalOutputSats.plus(txFee));
        if (remainderSats.isGreaterThanOrEqualTo(this.dustLimit)) {
            transactionBuilder.addOutput(bchjs.Address.toLegacyAddress(changeAddress), parseInt(remainderSats));
        }
        else {
            const remainderSatsNum = remainderSats.toNumber();
            if (remainderSatsNum < 0) {
